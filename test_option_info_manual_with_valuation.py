...
# Test values (AAPL, use first available expiration and a reasonable strike)
symbol = 'AAPL'
ticker = yf.Ticker(symbol)
available_expirations = ticker.options
if not available_expirations:
    print('No available expirations for', symbol)
    exit(1)
...
try:
    ...
    options = opt_chain.calls  # Test with calls by default
    option_type = 'call'
except Exception as e:
    print(f'Error fetching option chain: {e}')
    exit(1)

# Pick a strike near the median
strikes = options['strike'].values
if len(strikes) == 0:
    print('No strikes found for expiration', expiration)
    exit(1)
strike = float(np.median(strikes))

# Find the specific contract
row = options[options['strike'] == strike]
if row.empty:
    # If exact median not found, pick the closest
...
S = ticker.history(period='1d')['Close'].iloc[-1]  # Fixed the deprecation warning
K = strike
